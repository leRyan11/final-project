...
Types involved
BartSnakeGame checks whether the snake head and card are on the same square, updates score, grows the snake, and moves the card.
...
Inputs and outputs
Input: the player moves with w, a, s, or d. The game checks the snake head against the Clipper card after each update.
...
The card is on the right edge of the map and collection still succeeds.
